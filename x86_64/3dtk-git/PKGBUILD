...
prepare() {
	...
	sed -i "s/ Boost::system//g" 3rdparty/cctag/src/CMakeLists.txt
	sed -i "s/find_package(Eigen3 \${CCTAG_EIGEN_REQUIRED_VERSION} REQUIRED)/find_package(Eigen3 REQUIRED)/" 3rdparty/cctag/CMakeLists.txt
	sed -i "s/serialization;system;thread/serialization;thread/" 3rdparty/cctag/CMakeLists.txt
...
build() {
	...
	cmake -B build -GNinja -Wno-dev -DOpenCV_DIR=/usr/lib/cmake/opencv4 -DCMAKE_CXX_FLAGS="-isystem/usr/include/opencv4 -include cassert" \
		-DWITH_WXWIDGETS=OFF \

...
makedepends=("zstd=${_zstdver}" 'bash' 'bc' 'boost' 'boost-libs' 'bzip2' 'c-ares' 'cmake' 'coreutils' 'coffeescript'
             ...
             'libuv' 'libxml2' 'librdkafka1' 'libpciaccess' 'lsb-release' 'lz4' 'ncurses'
...
prepare() {
  ...
  # https://aur.archlinux.org/pkgbase/ceph-bin#comment-899146
  sed -i 's/\.to_string()//g' -i src/rgw/rgw_asio_client.cc
  sed -i 's/remote_endpoint\.address())/remote_endpoint.address().to_string())/g' -i src/rgw/rgw_asio_client.cc
}
